...
- **Diff output isn't a source patch.** `flate diff` defaults to dyff path-keyed syntax (`@@ <path> @@`), which GitHub's diff lexer renders natively. `-o diff` emits a standard unified diff, but of the *rendered* manifests on each side — neither form applies with `patch` / `git apply` against your repo. Use the rendered output of `flate build` if you need a literal patch.
